...
export default function Calendar ({
	...
	*/
	...
	let forwardMonth = () => {
		...
		set_postsPerDate([]);
		set_nextClass('selected'); 
		set_currentClass('off');

		setTimeout(() => {
			set_nextClass('nextEnd'); //rm
		}, 600)
		...
		setTimeout(() => {
			if(selectedDate.month + 1 > 11) {
				...
				set_prevMonth(cal.monthsAbrv[11]); //rm

				setTimeout(() => {
					set_nextMonth(cal.monthsAbrv[1]) //rm
				}, 300)
				...
				set_prevMonth(cal.monthsAbrv[10]) //rm
				set_currentMonth(cal.monthsAbrv[11]);
				setTimeout(() => {
					set_nextMonth(cal.monthsAbrv[0]); //rm
				}, 300)
			} 
			else {
				setSelectedDate({
				...
				set_prevMonth(cal.monthsAbrv[selectedDate.month]) //rm
				setTimeout(() => {
					set_nextMonth(cal.monthsAbrv[selectedDate.month + 2]) //rm
				}, 300)
			}
			/*Return Calendar*/
			set_calClass('return');
	...
	let backwardMonth = () => {
		...
		set_postsPerDate([]);
		set_prevClass('selected'); 
		set_currentClass('off');

		setTimeout(() => {
			set_prevClass('nextEnd'); //rm
		}, 600)
	...
	*/
	...
	return (
		...
		<div id="monthChart" className={`${sectionClass.calendar}`}>
			...
			<div id="header">
				...
				<button id="prev" className={prevClass} onClick={ async()=> {
				...
				<button id="next" className={nextClass} onClick={	async()=> {
